...
void FAR Fade (HDC hdc, RECT rc, long lTopColor, long lBottomColor, USHORT uMethod)
   ...
   for (i = 0; i <= uSTRIPES; i++)
      ...
      switch (uMethod)
         {
         case VERTICAL:
            rc.top    = (short) (i * cyClient / uSTRIPES);
            rc.bottom = rc.top + cyClient / uSTRIPES + 1;
            break;
         case HORIZONTAL:
            rc.left   =(short) (i * cxClient / uSTRIPES);
            rc.right  =rc.left + cxClient / uSTRIPES + 1;
            break;
         case VERTCTR:
            rc.top    = (short) ((long)i * (long) cyClient / (long)(uSTRIPES * 2));
            rc.bottom = rc.top + cyClient / (uSTRIPES * 2) + 1;
            break;
         case HORZCTR:
            rc.left   =(short) ((long)i * (long)cxClient / (long)(uSTRIPES * 2));
            rc.right  =rc.left + cxClient / (uSTRIPES * 2) + 1;
            break;
         }
      ...
      if (uMethod == VERTCTR)
         {
         rc.top    = (short) ((long)(uSTRIPES * 2 - 1 -i) * (long)cyClient / (long)(uSTRIPES * 2));
         rc.bottom = rc.top + cyClient / (uSTRIPES * 2) + 1;
         FillRect (hdc, &rc, hBrush);
         }
      if (uMethod == HORZCTR)
         {
         rc.left   =(short) ((long)(uSTRIPES * 2 - 1 -i) * (long)cxClient / (long)(uSTRIPES * 2));
         rc.right  =rc.left + cxClient / (uSTRIPES * 2) + 1;
         FillRect (hdc, &rc, hBrush);
         }
      DeleteObject (hBrush);
